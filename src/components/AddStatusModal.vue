<template>
  <div class="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" @click.self="$emit('close')">
    <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-lg border border-neutral-200/70 dark:border-neutral-700/50 p-5 sm:p-6 w-full max-w-sm">
      <h2 class="text-lg font-bold text-neutral-800 dark:text-white mb-4">เพิ่มสถานะใหม่</h2>
      ...
      <form @submit.prevent="handleAdd" class="space-y-4">
        <div>
          <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">ชื่อสถานะ</label>
          <input
            v-model="name"
            type="text"
            required
        ...
        <div>
          <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">สี</label>
