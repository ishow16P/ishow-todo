<template>
  <div class="max-w-4xl mx-auto px-4 py-6 sm:py-8">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 sm:mb-8">
      <h1 class="text-xl sm:text-2xl font-bold text-neutral-800 dark:text-white">โปรเจคของฉัน</h1>
      <button
    ...
    <div v-if="projectStore.loading" class="text-center text-neutral-400 dark:text-neutral-500 py-12">กำลังโหลด...</div>

    <div v-else-if="projectStore.projects.length === 0" class="text-center py-16 sm:py-20">
      <p class="text-neutral-400 dark:text-neutral-500 text-lg mb-4">ยังไม่มีโปรเจค</p>
      <button
        @click="showCreate = true"
    ...
    <template v-else>
      ...
      <div v-if="filteredProjects.length === 0" class="text-center py-12">
        <p class="text-neutral-400 dark:text-neutral-500">ไม่พบโปรเจคที่ค้นหา</p>
      </div>
      ...
      >
        ...
        <router-link :to="`/projects/${project._id}`" class="block">
          ...
          <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1 line-clamp-2">{{ project.description || 'ไม่มีคำอธิบาย' }}</p>
    ...
    <div v-if="showCreate" class="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4" @click.self="showCreate = false">
      <div class="bg-white dark:bg-neutral-800 rounded-xl shadow-lg border border-neutral-200/70 dark:border-neutral-700/50 p-5 sm:p-6 w-full max-w-md">
        <h2 class="text-lg font-bold text-neutral-800 dark:text-white mb-4">สร้างโปรเจคใหม่</h2>
        <form @submit.prevent="handleCreate" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">ชื่อโปรเจค</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">คำอธิบาย (ไม่บังคับ)</label>
            <textarea
...
const filteredProjects = computed(() => {
  ...
  return [...results].sort((a, b) => {
    ...
    // newest: createdAt desc, fallback to _id desc
...
async function handleDelete(project) {
  const { isConfirmed } = await Swal.fire({
    title: 'ลบโปรเจค?',
    text: `"${project.name}" ข้อมูลทั้งหมดจะถูกลบและไม่สามารถกู้คืนได้`,
    icon: 'warning',
